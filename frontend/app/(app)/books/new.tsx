...
export default function NewBookScreen() {
  ...
  async function handleFetchFromLink() {
    ...
    setFetching(true);
    setError('');
    const result = await bookApi.fetchMetadataPreview(sourceUrl.trim());
    setFetching(false);
    if (result.ok) {
      ...
      if (!result.data.title && !result.data.coverImageUrl) {
        setError('Could not find a name or photo at that link — enter them manually.');
      }
    } else {
  ...
  async function handleCreate() {
    ...
    if (result.ok) {
      ...
      setError(result.error.message ?? 'Failed to add book.');
    }
  }
  ...
  return (
    <Screen scroll padded>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        />
        <Button
          label={fetching ? 'Fetching…' : 'Fetch Name & Photo from Link'}
